...
const fnRegisterMember = ()=>{
  $.ajax({
    ...
    data: JSON.stringify({                   // 보내는 데이터 (stringify : 문자열 형식의 JSON 데이터)
      'email': jqEmail.val(),
      'name': jqName.val(),
      'gender': $(':radio:checked').val(),   // 체크가 된 항목의 데이터를 가져오겠다(제이쿼리)
  ...
  }).done(resData=>{                // resData = {"insertCount": 2} : member 1 + address 1
