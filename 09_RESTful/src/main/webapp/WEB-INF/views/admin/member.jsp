...
<body>
  ...
  <div>
    ...
    <div>
      <button id="btn-init" type="reset">초기화</button>
  ...
  <script>
    ...
    const fnInit = ()=>{
    	...
    	email.val('');
    	mName.val('');
    	$('#none').prop('checked', true);   // 빈문자열이 불가능한 라디오는 none 을 checked로 한다.
    ...
    const fnRegisterMember = ()=>{
    	$.ajax({
    		...
    		data: JSON.stringify({           // 보내는 데이터 (문자열 형식의 JSON 데이터)
    			'email': email.val(),
    			'name': mName.val(),
    			'gender': $(':radio:checked').val(),  // 체크된 라디오의 값
    	...
    	}).done(resData=>{  // 회원가입 성공 : resData = {"insertCount": 2}
    	...
    	}).fail(jqXHR=>{    // 실패(유니크 관련된 오류 : 같은 이메일이 2개일 수 없다)
